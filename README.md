...
Throttle과 Debounce는 이벤트 핸들러가 많은 연산을 하는 경우에 대해 제약을 걸어 시간이 지남에 따라 함수를 몇 번 실행 할지를 제어하는 유사한 기술이지만 서로 다른 기술이다. 

예)
- 무거운 계산 및 기타 DOM 조작)을 수행
- 이벤트 핸들러의 과도한 횟수가 발생하는 것

  
사용 예)
- 사용자가 창 크기 조정을 멈출 때까지 기다렸다가 resizing event 사용
- 사용자가 키보드 입력을 중지(예: 검색창) 할 때까지 ajax 이벤트를 발생시키지 않기 위해 사용
- 페이지의 스크롤 위치를 측정하고 최대 50ms 마다 응답하기를 바랄 경우에 사용
- 앱에서 요소를 드래그 할 때 좋은 성능을 보장하기 위해 사용
...
- 5분 안에 컴포넌트가 마운트 되면 5분 안에 컴포넌트가 마운트 됏기 때문에 캐시에 있던 데이터를 그대로 사용 합니다.</br>
